            ...
            <div class="flex flex-wrap items-center justify-center space-x-6">
                <Suspense fallback=move || view! { <div></div> }>
                    {move || {
                        ...
                        let github_url = s.github_url.clone();
                        let x_url = s.x_url.clone();
                        let linkedin_url = s.linkedin_url.clone();
                        view! {
                            <div class="flex items-center space-x-4">
                                {if !github_url.is_empty() {
                                    Some(view! {
                                        <a href=github_url target="_blank" rel="noopener noreferrer" class="text-slate-400 hover:text-primary transition-colors">
                                ...
                                {if !x_url.is_empty() {
                                    Some(view! {
                                        <a href=x_url target="_blank" rel="noopener noreferrer" class="text-slate-400 hover:text-primary transition-colors">
                                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                ...
                                {if !linkedin_url.is_empty() {
                                    Some(view! {
                                        <a href=linkedin_url target="_blank" rel="noopener noreferrer" class="text-slate-400 hover:text-primary transition-colors">
                                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
