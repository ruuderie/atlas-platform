...
// ── Public dispatcher ──────────────────────────────────────────────────────
// Each variant is implemented as a private component below. Adding a new
// tenant mode means adding a new branch here and a new sub-component —
// existing variants are untouched.

#[component]
pub fn ProfileHeaderBlock(data: ProfileHeaderBlockData) -> impl IntoView {
    if use_kami_mode() {
        view! { <KamiProfileHeader data=data /> }.into_any()
    } else {
        view! { <DefaultProfileHeader data=data /> }.into_any()
    }
}
...
#[component]
fn KamiProfileHeader(data: ProfileHeaderBlockData) -> impl IntoView {
    view! {
                    ...
                    view! {
                        <div class="flex flex-wrap justify-center gap-x-6 gap-y-1 text-xs text-[#6b6a64] jetbrains mb-6">
                            ...
                            {if let Some(linkedin) = data.contact.linkedin_url.clone() {
                                view! { <a href={linkedin} target="_blank" class="hover:text-[#1B365D] transition-colors underline">"LinkedIn"</a> }.into_any()
                            } else { view! {}.into_any() }}
                        </div>
                    }.into_any()
                } else { view! {}.into_any() }}
...
#[component]
fn DefaultProfileHeader(data: ProfileHeaderBlockData) -> impl IntoView {
    view! {
                    ...
                    <div class="flex-grow space-y-4">
                        <div>
                            ...
                            {if let Some(title) = data.title {
                                view! { <div class="text-xl md:text-2xl text-primary font-medium">{title}</div> }.into_any()
                            } else { view! {}.into_any() }}
                        </div>
                            ...
                            view! {
                                <div class="flex flex-wrap gap-x-6 gap-y-2 text-sm text-on-surface-variant pt-2">
                                    ...
                                    {if let Some(linkedin) = data.contact.linkedin_url {
                                        view! { <a href={linkedin} target="_blank" class="flex items-center gap-1.5 hover:text-primary transition-colors hover:underline">"LinkedIn"</a> }.into_any()
                                    } else { view! {}.into_any() }}
                                </div>
                            }.into_any()
                        } else { view! {}.into_any() }}
