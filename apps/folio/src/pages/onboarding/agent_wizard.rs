...
const STEPS: &[WizardStepDesc] = &[
    WizardStepDesc { id: "profile",     label: "Profile & License",       skippable: false },
    WizardStepDesc { id: "specialties", label: "Specialties & Markets",   skippable: false },
    WizardStepDesc { id: "brokerage",   label: "Brokerage Assignment",    skippable: false },
    WizardStepDesc { id: "tools",       label: "Tools & Notifications",   skippable: false },
];
...
pub fn AgentWizard() -> impl IntoView {
    ...
    let next_label  = Signal::derive(move || if is_last.get() { "Go to Agent Dashboard" } else { "Continue" });
