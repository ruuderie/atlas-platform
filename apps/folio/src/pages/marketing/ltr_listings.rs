...
pub fn LtrListings() -> impl IntoView {
    ...
    let q = query.get();
    let init_city = q.get("city").cloned().unwrap_or_default();
    let init_beds = q.get("beds").cloned().unwrap_or_default();
    let init_rent = q.get("max_rent").cloned().unwrap_or_default();
