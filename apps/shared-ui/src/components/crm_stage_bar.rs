...
pub fn CrmStageBar(
    ...
    let get_stage_classes = move |color: &str, is_active: bool| -> String {
        ...
        if is_active {
            match color {
                "blue" => format!(
                    "{} bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-500/20 scale-[1.02] z-10",
                    base
                ),
                "purple" => format!(
                    "{} bg-purple-600 border-purple-600 text-white shadow-lg shadow-purple-500/20 scale-[1.02] z-10",
                    base
                ),
                "indigo" => format!(
                    "{} bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-500/20 scale-[1.02] z-10",
                    base
                ),
                "orange" => format!(
                    "{} bg-orange-500 border-orange-500 text-white shadow-lg shadow-orange-500/20 scale-[1.02] z-10",
                    base
                ),
                "emerald" => format!(
                    "{} bg-emerald-600 border-emerald-600 text-white shadow-lg shadow-emerald-500/20 scale-[1.02] z-10",
                    base
                ),
                "rose" => format!(
                    "{} bg-rose-600 border-rose-600 text-white shadow-lg shadow-rose-500/20 scale-[1.02] z-10",
                    base
                ),
                _ => format!(
                    "{} bg-slate-600 border-slate-600 text-white shadow-lg shadow-slate-500/20 scale-[1.02] z-10",
                    base
                ), // slate / fallback
            }
        } else {
            match color {
                "blue" => format!(
                    "{} bg-surface-container border-outline-variant/30 text-blue-500 hover:bg-blue-50/10 hover:border-blue-400/30",
                    base
                ),
                "purple" => format!(
                    "{} bg-surface-container border-outline-variant/30 text-purple-500 hover:bg-purple-50/10 hover:border-purple-400/30",
                    base
                ),
                "indigo" => format!(
                    "{} bg-surface-container border-outline-variant/30 text-indigo-500 hover:bg-indigo-50/10 hover:border-indigo-400/30",
                    base
                ),
                "orange" => format!(
                    "{} bg-surface-container border-outline-variant/30 text-orange-500 hover:bg-orange-50/10 hover:border-orange-400/30",
                    base
                ),
                "emerald" => format!(
                    "{} bg-surface-container border-outline-variant/30 text-emerald-500 hover:bg-emerald-50/10 hover:border-emerald-400/30",
                    base
                ),
                "rose" => format!(
                    "{} bg-surface-container border-outline-variant/30 text-rose-500 hover:bg-rose-50/10 hover:border-rose-400/30",
                    base
                ),
                _ => format!(
                    "{} bg-surface-container border-outline-variant/30 text-on-surface-variant hover:bg-slate-100/10 hover:border-slate-400/30",
                    base
                ), // slate / fallback
            }
        }
