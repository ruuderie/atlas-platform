                ...
                match shared_ui::auth::atlas_auth::server_fns::verify_magic_link(active_token).await
                {
                    Ok(token_str) => {
                        ...
                        match crate::api::auth::get_session().await {
                            Ok(user) => {
                                ...
                                toast_clone.show_toast(
                                    "Auth",
                                    "Token consumed! Please register a new Passkey immediately.",
                                    "success",
                                );
                    ...
                    Err(e) => {
                        ...
                        if e_str.contains("token_already_used") {
                            error_msg
                                .set(Some("This magic link has already been used.".to_string()));
                            ...
                            error_msg.set(Some(
                                "This magic link is invalid or does not exist.".to_string(),
                            ));
                        } else {
