...
#[component]
pub fn DirectoryTypeCreate() -> impl IntoView {
        ...
        async move {
            ...
            match req.send().await {
                Ok(res) if res.status().is_success() => {
                    ...
                    nav("/directory-types", leptos_router::NavigateOptions::default());
                }
                Ok(res) => {
    ...
    view! {
        <div class="max-w-3xl mx-auto space-y-6 pt-8">
            <header class="mb-8">
                <a href="/directory-types" class="text-sm text-muted-foreground hover:text-foreground mb-4 inline-block">"← Back"</a>
            ...
            >
                ...
                <div>
                    ...
                    <textarea 
                        class="w-full bg-surface-container-highest border border-outline/20 text-on-surface text-sm rounded-lg focus:ring-primary focus:border-primary block p-3"
                        placeholder="Describe the purpose of this directory type"
                        rows="3"
