...
fn get_mock_products() -> Vec<PlatformProductModel> {
    vec![
        PlatformProductModel {
            id: Uuid::parse_str("11111111-1111-1111-1111-111111111111").unwrap(),
            name: "Atlas PM — Residential".to_string(),
            slug: "pm-residential".to_string(),
            tagline: Some("Standard residential property management engine".to_string()),
            status: "active".to_string(),
            marketing_page_cms_id: None,
            deploy_hook_url: Some("https://api.cloudflare.com/client/v4/pages/webhooks/deploy_pm".to_string()),
            ...
            pre_order_sold: 0,
            waitlist_count: 0,
            sentinel_tenant_id: None,
            apex_domain: Some("pm.atlas.app".to_string()),
            apex_domain_verified: true,
        ...
        PlatformProductModel {
            ...
            slug: "miami-str".to_string(),
            tagline: Some("Short term rental manager custom built for Miami market".to_string()),
            status: "active".to_string(),
            marketing_page_cms_id: None,
            deploy_hook_url: Some("https://api.cloudflare.com/client/v4/pages/webhooks/deploy_str".to_string()),
            launch_mode: "active".to_string(),
            pre_order_enabled: false,
            pre_order_price_cents: None,
            pre_order_currency: "USD".to_string(),
            ...
            waitlist_count: 0,
            sentinel_tenant_id: None,
            apex_domain: Some("miami.atlas.app".to_string()),
            apex_domain_verified: true,
            created_at: "".to_string(),
            updated_at: "".to_string(),
        },
        PlatformProductModel {
            ...
            slug: "atlas-commercial".to_string(),
            tagline: Some("Enterprise tier commercial lease coordinator".to_string()),
            status: "beta".to_string(),
            marketing_page_cms_id: None,
            deploy_hook_url: None,
            launch_mode: "beta".to_string(),
            pre_order_enabled: true,
            pre_order_price_cents: Some(150000),
            pre_order_currency: "USD".to_string(),
            stripe_price_id: None,
            pre_order_cap: Some(100),
            pre_order_sold: 42,
            waitlist_count: 61,
            sentinel_tenant_id: None,
            apex_domain: Some("commercial.atlas.app".to_string()),
            apex_domain_verified: false,
            created_at: "".to_string(),
            updated_at: "".to_string(),
        },
        PlatformProductModel {
            id: Uuid::parse_str("44444444-4444-4444-4444-444444444444").unwrap(),
            name: "Atlas Wholesale — USVI".to_string(),
            slug: "wholesale-usvi".to_string(),
            tagline: Some("B2B wholesale inventory aggregator".to_string()),
            status: "pre-launch".to_string(),
            marketing_page_cms_id: None,
            deploy_hook_url: None,
            launch_mode: "waitlist".to_string(),
            pre_order_enabled: false,
            pre_order_price_cents: None,
            pre_order_currency: "USD".to_string(),
            stripe_price_id: None,
            ...
            waitlist_count: 24,
            sentinel_tenant_id: None,
            apex_domain: Some("usvi.atlas.app".to_string()),
            apex_domain_verified: false,
            created_at: "".to_string(),
            updated_at: "".to_string(),
        },
        PlatformProductModel {
            ...
            name: "Atlas STR — Brazil".to_string(),
            slug: "brazil-str".to_string(),
            tagline: Some("AI localized short term rental portal".to_string()),
            status: "ai".to_string(),
            marketing_page_cms_id: None,
            deploy_hook_url: None,
            launch_mode: "beta".to_string(),
            pre_order_enabled: false,
            pre_order_price_cents: None,
            pre_order_currency: "BRL".to_string(),
            stripe_price_id: None,
            pre_order_cap: None,
            pre_order_sold: 0,
            waitlist_count: 12,
            sentinel_tenant_id: None,
            apex_domain: Some("brazil.atlas.app".to_string()),
            apex_domain_verified: false,
            created_at: "".to_string(),
            updated_at: "".to_string(),
        },
        PlatformProductModel {
            ...
            name: "Atlas PM — LATAM".to_string(),
            slug: "latam-pm".to_string(),
            tagline: Some("Residential property manager for LATAM market".to_string()),
            status: "draft".to_string(),
            marketing_page_cms_id: None,
            deploy_hook_url: None,
            launch_mode: "draft".to_string(),
            pre_order_enabled: false,
            pre_order_price_cents: None,
            pre_order_currency: "USD".to_string(),
            stripe_price_id: None,
            ...
            waitlist_count: 0,
            sentinel_tenant_id: None,
            apex_domain: Some("latam.atlas.app".to_string()),
            apex_domain_verified: false,
            created_at: "".to_string(),
            updated_at: "".to_string(),
        },
...
fn merge_real_and_mock_products(real: Vec<PlatformProductModel>) -> Vec<PlatformProductModel> {
    let mocks = get_mock_products();
    if real.is_empty() {
        return mocks;
    }
    ...
    for r in &real {
        ...
        for (idx, m) in mocks.iter().enumerate() {
            if m.slug == r.slug || m.name.to_lowercase() == r.name.to_lowercase() {
                found_idx = Some(idx);
                break;
            }
        }
        if let Some(idx) = found_idx {
            let mut matched = mocks[idx].clone();
            matched.id = r.id;
            matched.tagline = r.tagline.clone();
            matched.status = r.status.clone();
            matched.deploy_hook_url = r.deploy_hook_url.clone();
            matched.marketing_page_cms_id = r.marketing_page_cms_id;
            matched.launch_mode = r.launch_mode.clone();
            matched.pre_order_enabled = r.pre_order_enabled;
            matched.pre_order_price_cents = r.pre_order_price_cents;
            matched.pre_order_currency = r.pre_order_currency.clone();
            matched.stripe_price_id = r.stripe_price_id.clone();
            matched.pre_order_cap = r.pre_order_cap;
            matched.pre_order_sold = r.pre_order_sold;
            matched.waitlist_count = r.waitlist_count;
            matched.sentinel_tenant_id = r.sentinel_tenant_id;
            matched.apex_domain = r.apex_domain.clone();
            matched.apex_domain_verified = r.apex_domain_verified;
            result.push(matched);
    ...
    for (idx, m) in mocks.into_iter().enumerate() {
...
pub fn BillingProducts() -> impl IntoView {
    ...
    let products_list = Signal::derive(move || {
        merge_real_and_mock_products(db_products.get().unwrap_or_default())
    });
