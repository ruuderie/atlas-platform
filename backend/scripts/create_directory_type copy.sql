...
-- Create networks for Healthcare

INSERT INTO network_type (id, name, description, created_at, updated_at)
VALUES
  (gen_random_uuid(), 'Healthcare', 'Network for healthcare services', NOW(), NOW())
...
FROM
  (VALUES
    ('Smart Health', 'healthcarenetwork.com', 'Your guide to healthcare services')
  ) AS d(name, domain, description),
  network_type dt
