...
use crate::models::listing::{ListingCreate, ListingStatus, ListingUpdate, PaginatedListings};
use axum::{
    Router,
...
pub fn authenticated_routes() -> Router<DatabaseConnection> {
    Router::new()
        ...
        .route(
            ...
            get(get_my_listings).post(create_my_listing),
        )
        ...
        .route(
            "/api/me/accounts/{account_id}/listings",
            get(get_account_listings),
        )
...
pub async fn create_listing(
    ...
    println!(
        "TEST LOG: from create_listing and current_user: {:?}",
        current_user
    );
    println!(
        "TEST LOG: from create_listing and input.profile_id: {:?}",
        input.profile_id
    );
    ...
    let user_account_exists = UserAccount::find()
        ...
        .is_some();
    println!(
        "TEST LOG: from create_listing and user_account_exists: {:?}",
        user_account_exists
    );
    ...
    println!(
        "TEST LOG: from create_listing and new_listing: {:?}",
        new_listing
    );
...
pub async fn update_listing(
    ...
    if user_account_exists.is_none() {
        ...
        if !is_platform_admin {
            println!(
                "TEST LOG: from update_listing and user_account_exists: {:?}",
                user_account_exists
            );
            return Err(StatusCode::FORBIDDEN);
        }
    ...
    if let Some(title) = input.title.as_ref() {
        listing_active_model.title = Set(title.clone());
    }
    if let Some(description) = input.description.as_ref() {
        listing_active_model.description = Set(description.clone());
    }
    if let Some(category_id) = input.category_id {
        listing_active_model.category_id = Set(Some(category_id));
    }
    if let Some(listing_type) = input.listing_type.as_ref() {
        listing_active_model.listing_type = Set(listing_type.clone());
    }
    if let Some(price) = input.price {
        listing_active_model.price = Set(Some(price));
    }
    if let Some(price_type) = input.price_type.as_ref() {
        listing_active_model.price_type = Set(Some(price_type.clone()));
    }
    if let Some(country) = input.country.as_ref() {
        listing_active_model.country = Set(Some(country.clone()));
    }
    if let Some(state) = input.state.as_ref() {
        listing_active_model.state = Set(Some(state.clone()));
    }
    if let Some(city) = input.city.as_ref() {
        listing_active_model.city = Set(Some(city.clone()));
    }
    if let Some(neighborhood) = input.neighborhood.as_ref() {
        listing_active_model.neighborhood = Set(Some(neighborhood.clone()));
    }
    if let Some(latitude) = input.latitude {
        listing_active_model.latitude = Set(Some(latitude));
    }
    if let Some(longitude) = input.longitude {
        listing_active_model.longitude = Set(Some(longitude));
    }
    if let Some(additional_info) = input.additional_info {
        listing_active_model.additional_info = Set(Some(additional_info));
    }
    ...
    println!(
        "TEST LOG: from update_listing and updated_listing: {:?}",
        updated_listing
    );
...
pub async fn search_listings(
    ...
    let mut query =
        Listing::find().filter(listing::Column::Title.like(format!("%{}%", q.q).as_str()));
