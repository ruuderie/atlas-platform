...
pub fn authenticated_routes_raw() -> Router<DatabaseConnection> {
    Router::new()
        .route(
            "/api/folio/properties/{id}/value",
            post(log_property_value),
        )
        .route(
...
pub async fn log_property_value(
    ...
    let source = match PropertyValueSource::try_from(body.source.clone()) {
        Ok(s)  => s,
        Err(_) => {
    ...
    let currency = body.currency_code.as_deref().unwrap_or("USD").to_uppercase();
    ...
    match db.execute(
        ...
        Ok(_) => {
            ...
            (StatusCode::CREATED, axum::Json(LogValueResponse { id: entry_id })).into_response()
        }
        Err(e) => {
            ...
            let entries: Vec<ValueHistoryEntry> = rows
                ...
                .filter_map(|row| {
                    Some(ValueHistoryEntry {
                        ...
                        created_at:    row.try_get::<chrono::DateTime<chrono::Utc>>("", "created_at")
                            .ok()
