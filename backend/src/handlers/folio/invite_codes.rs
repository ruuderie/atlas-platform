...
pub fn public_routes() -> Router<DatabaseConnection> {
    Router::new().route("/api/folio/invite/resolve/{code}", get(resolve_code))
}
...
pub fn authenticated_routes() -> Router<DatabaseConnection> {
    Router::new()
        ...
        .route(
            "/api/folio/invite-codes/by-code/{code}/accept",
            post(accept_code_by_str),
        )
}
...
async fn list_codes(State(db): State<DatabaseConnection>, headers: HeaderMap) -> impl IntoResponse {
    let token = match extract_bearer(&headers) {
        Some(t) => t,
        None => {
            return (
        ...
        Ok(rows) => {
            let codes: Vec<serde_json::Value> = rows
                ...
                .map(|r| {
                    ...
                    let expires_at: Option<String> = r
                        .try_get::<Option<chrono::DateTime<Utc>>>("", "expires_at")
                        .ok()
                        .flatten()
...
async fn patch_code(
    ...
    if let Some(max_uses) = input.max_uses {
        sets.push(format!("max_uses = ${}", idx));
        params.push(max_uses.into());
        idx += 1;
    }
...
async fn accept_code(
    ...
    for outcome in [
        ...
        if let Err(e) =
            crate::services::program_service::ProgramService::complete_outcomes_for_invite_code(
                &db,
                id,
                outcome,
                accepting_user_id,
...
async fn accept_code_by_str(
    ...
    accept_code(Path(id), State(db), headers)
        .await
        .into_response()
}
