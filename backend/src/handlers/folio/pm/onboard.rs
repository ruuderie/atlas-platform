...
use crate::entities::{atlas_account, atlas_user_app_roles, platform_invite, tenant_setting, user};
use crate::services::auth_service::AuthService;

// ── Public route ──────────────────────────────────────────────────────────────

pub fn public_routes_raw() -> Router<DatabaseConnection> {
    Router::new().route("/api/folio/pm/onboard", post(submit_pmc_onboard))
}
...
async fn submit_pmc_onboard(
    ...
    let invite = platform_invite::Entity::find_by_id(body.invite_id)
        ...
        .await
        .map_err(|e| {
            tracing::error!("pmc/onboard: DB error fetching invite: {e}");
            StatusCode::INTERNAL_SERVER_ERROR
        })?
        .ok_or_else(|| {
            tracing::warn!("pmc/onboard: invite {} not found", body.invite_id);
    ...
    if !is_pmc {
        tracing::warn!(
            "pmc/onboard: invite {} has wrong app_role: {:?}",
            body.invite_id,
            invite.app_role
        );
    ...
    let user_id = if let Some(u) = existing_user {
        ...
        new_user
            ...
            .await
            .map_err(|e| {
                tracing::error!("pmc/onboard: failed to create user: {e}");
                StatusCode::INTERNAL_SERVER_ERROR
            })?
            .id
    };
    ...
    new_account.insert(&txn).await.map_err(|e| {
        tracing::error!("pmc/onboard: failed to create account: {e}");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
    ...
    role_assignment.insert(&txn).await.map_err(|e| {
        tracing::error!("pmc/onboard: failed to assign role: {e}");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
    ...
    platform_invite::Entity::delete_by_id(body.invite_id)
        ...
        .await
        .map_err(|e| {
            tracing::error!("pmc/onboard: failed to delete invite: {e}");
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    ...
    let magic_link_url = match AuthService::create_magic_link(&db, &email_lower).await {
        Ok(tok) => format!(
            "{}/magic-login?token={}&next=/pmc/dashboard",
            frontend_url, tok.token
        ),
