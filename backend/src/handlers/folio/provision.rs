...
fn validate_invite(input: &ProvisionInviteInput) -> Result<FolioRole, String> {
    ...
    let role = FolioRole::try_from(input.app_role.as_str()).map_err(|e| e.to_string())?;

    match role {
        // StrGuest: must know which property the guest is booking
        FolioRole::StrGuest => {
            if input.asset_id.is_none() {
                return Err("str_guest invites require an asset_id. \
                     The guest must be linked to a specific STR-eligible property. \
                     Optionally include a booking_id if a reservation already exists."
                    .to_string());
            }
        }
...
async fn send_provision_email(
    ...
    let msg = match Message::builder()
        ...
        Ok(m) => m,
        Err(e) => {
            tracing::error!("provision_invite: failed to build email: {e:?}");
            return;
        }
    };
    ...
    tokio::task::spawn_blocking(move || match mailer.send(&msg) {
...
pub async fn provision_self(
    ...
    if let Err(e) = db
        .execute(Statement::from_string(DatabaseBackend::Postgres, role_sql))
        .await
    {
    ...
    if let Some(inv_id) = body.invite_id {
        ...
        let _ = db
            .execute(Statement::from_string(DatabaseBackend::Postgres, g22_sql))
            .await;
    }
