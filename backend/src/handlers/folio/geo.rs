//! # G01 Geo HTTP handlers — Folio (PM-tier)
//!
//! Exposes PostGIS spatial queries as REST endpoints for the Folio app.
//!
//! ## Routes (all authenticated)
//!
//! | Method | Path                                      | Description                              |
//! |--------|-------------------------------------------|------------------------------------------|
//! | GET    | /api/folio/geo/leads/radius               | Leads within radius of a point           |
//! | GET    | /api/folio/geo/leads/nearest              | N nearest leads to a point               |
//! | POST   | /api/folio/geo/leads/{id}/geocode         | Set geo_point on a lead                  |
//! | GET    | /api/folio/geo/accounts/radius            | Accounts within radius of a point        |
//! | POST   | /api/folio/geo/accounts/{id}/geocode      | Set geo_point on an account              |
//! | GET    | /api/folio/geo/service-areas/contains     | Service areas containing a point         |
//! | GET    | /api/folio/geo/service-areas/radius       | Service areas near a point               |
//! | GET    | /api/folio/geo/status                     | PostGIS availability health check        |
...
use crate::services::pm::geo::GeoService;

// ── Route constructor ─────────────────────────────────────────────────────────

pub fn authenticated_routes_raw() -> Router<DatabaseConnection> {
    Router::new()
        .route("/api/folio/geo/status", get(geo_status))
        .route("/api/folio/geo/leads/radius", get(leads_radius))
        .route("/api/folio/geo/leads/nearest", get(leads_nearest))
        .route("/api/folio/geo/leads/{id}/geocode", post(geocode_lead))
        .route("/api/folio/geo/accounts/radius", get(accounts_radius))
        .route("/api/folio/geo/accounts/{id}/geocode", post(geocode_account))
        .route("/api/folio/geo/service-areas/contains", get(service_areas_contains))
        .route("/api/folio/geo/service-areas/radius", get(service_areas_radius))
}
...
/// GET /api/folio/geo/status — PostGIS availability check.
async fn geo_status(
...
/// GET /api/folio/geo/leads/radius?lng=&lat=&radius_m=
...
/// GET /api/folio/geo/leads/nearest?lng=&lat=&limit=
...
/// GET /api/folio/geo/accounts/radius?lng=&lat=&radius_m=
...
/// GET /api/folio/geo/service-areas/contains?lng=&lat=
...
/// GET /api/folio/geo/service-areas/radius?lng=&lat=&radius_m=
