...
impl From<atlas_account::Model> for AccountResponse {
    fn from(m: atlas_account::Model) -> Self {
        ...
        let account_type =
            AccountType::try_from(m.account_type.as_str()).unwrap_or(AccountType::Organization);
        let status = AccountStatus::try_from(m.status.as_str()).unwrap_or(AccountStatus::Active);
    ...
    let all: Vec<atlas_account::Model> = match query.all(&db).await {
        ...
        Err(e) => {
            ...
            return (
                ...
                JsonResponse(Vec::<AccountResponse>::new()),
            );
        }
    };
            ...
            all.into_iter()
                .filter(|a| {
                    a.name.to_lowercase().contains(&t)
                        ...
                        || a.industry
                            .as_deref()
                            .map(|i| i.to_lowercase().contains(&t))
                            .unwrap_or(false)
                        || a.company_email
                            .as_deref()
...
pub async fn get_account(
    ...
    match atlas_account::Entity::find_by_id(account_id).one(&db).await {
        Ok(Some(account)) => (
            StatusCode::OK,
            JsonResponse(Some(AccountResponse::from(account))),
        ),
...
pub async fn create_account(
    ...
    match new_account.insert(&db).await {
        ...
        Err(err) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                JsonResponse(None::<AccountResponse>),
            )
        }
    }
}
...
pub async fn update_account(
    ...
    if let Some(v) = payload.dba_name {
        active.dba_name = Set(Some(v));
    }
    if let Some(v) = payload.website {
        active.website = Set(Some(v));
    }
    if let Some(v) = payload.domain {
        active.domain = Set(Some(v));
    }
    if let Some(v) = payload.company_phone {
        active.company_phone = Set(Some(v));
    }
    if let Some(v) = payload.company_email {
        active.company_email = Set(Some(v));
    }
    if let Some(v) = payload.industry {
        active.industry = Set(Some(v));
    }
    if let Some(v) = payload.company_type {
        active.company_type = Set(Some(v));
    }
    if let Some(v) = payload.num_employees {
        active.num_employees = Set(Some(v));
    }
    if let Some(v) = payload.annual_revenue {
    ...
    if let Some(v) = payload.street_address {
        active.street_address = Set(Some(v));
    }
    if let Some(v) = payload.postal_code {
        active.postal_code = Set(Some(v));
    }
    if let Some(v) = payload.year_established {
        active.year_established = Set(Some(v));
    }
    if let Some(v) = payload.data_source {
    ...
    match active.update(&db).await {
        Ok(updated) => (
            StatusCode::OK,
            JsonResponse(Some(AccountResponse::from(updated))),
        ),
        Err(err) => {
...
pub async fn delete_account(
    ...
    match atlas_account::Entity::delete_by_id(account_id)
        .exec(&db)
        .await
    {
...
pub async fn add_user_to_account() -> StatusCode {
    StatusCode::NOT_IMPLEMENTED
}
pub async fn get_account_users() -> StatusCode {
    StatusCode::NOT_IMPLEMENTED
}
pub async fn get_account_ledger() -> StatusCode {
    StatusCode::NOT_IMPLEMENTED
}
pub async fn remove_user_from_account() -> StatusCode {
    StatusCode::NOT_IMPLEMENTED
}
pub async fn update_user_role_in_account() -> StatusCode {
    StatusCode::NOT_IMPLEMENTED
}
