...
pub fn authenticated_routes() -> Router<DatabaseConnection> {
    Router::new().route("/api/v1/telemetry/events", post(ingest_telemetry_events))
}
    ...
    let tenant_id_str = headers
        ...
        .ok_or_else(|| {
            (
                ...
                "Missing X-Tenant-Id header context".to_string(),
            )
        })?;

    let tenant_id = Uuid::parse_str(&tenant_id_str)
        .map_err(|_| (StatusCode::BAD_REQUEST, "Invalid tenant UUID".to_string()))?;
