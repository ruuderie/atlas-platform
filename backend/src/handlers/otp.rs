...
async fn store_otp(db: &DatabaseConnection, user_id: Uuid, code: &str) -> Result<(), StatusCode> {
...
fn send_otp_email(to_email: &str, code: &str) {
    ...
    let smtp_port = env::var("SMTP_PORT")
        ...
        .parse::<u16>()
        .unwrap_or(587);
    let smtp_from = env::var("SMTP_FROM").unwrap_or_else(|_| "noreply@folio.app".to_string());
  ...
  <p style="font-size:12px;color:#94a3b8;margin:0;">
    ...
    let plain_body =
        format!("Your Folio verification code is: {display_code}\n\nExpires in 5 minutes.");

    let msg = match Message::builder()
        .from(
            smtp_from
                .parse()
                .unwrap_or_else(|_| "noreply@folio.app".parse().unwrap()),
        )
        ...
        Ok(m) => m,
        Err(e) => {
            tracing::error!("otp: email build error: {:?}", e);
            return;
        }
    };
    ...
    let mailer = match SmtpTransport::relay(&smtp_server) {
        ...
        Err(e) => {
            tracing::error!("otp: SMTP relay error: {:?}", e);
            return;
        }
    };
...
async fn send_otp(
    ...
    if rate_limiter
        .check_rate_limit(&format!("otp:send:{}", req.email.to_lowercase()))
        .await
        .is_err()
    {
        return (
...
async fn verify_otp(
    ...
    let token_id: Uuid = match row {
        Some(r) => r.try_get::<Uuid>("", "id").map_err(|_| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "error": "Database error" })),
            )
