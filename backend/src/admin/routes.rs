...
use crate::handlers::{
    accounts, admin, cases, categories, contacts, customers, deals, files, leads, listings,
    profiles, templates,
};
use axum::{
    Router,
...
pub fn admin_routes_raw() -> Router<DatabaseConnection> {
    ...
    Router::new().merge({
        Router::new()
            ...
            .route(
                "/api/admin/users/{user_id}/toggle-admin",
            ...
            .route(
                "/api/admin/accounts",
                get(accounts::get_accounts).post(accounts::create_account),
            )
            .route(
            ...
            .route(
                "/api/admin/customers",
                get(customers::get_customers).post(customers::create_customer),
            )
            .route(
            ...
            .route(
                "/api/admin/leads",
                get(leads::get_leads).post(leads::create_lead),
            )
            .route(
            ...
            .route(
                "/api/admin/deals",
                get(deals::get_deals).post(deals::create_deal),
            )
            .route(
            ...
            .route(
                "/api/admin/deals/{deal_id}/files/{file_id}",
                post(deals::add_file_to_deal),
            )
            .route(
                "/api/admin/deals/{deal_id}/files",
                get(deals::get_deal_files),
            )
            //Contact management
            .route(
                "/api/admin/contacts",
                get(contacts::get_contacts).post(contacts::create_contact),
            )
            .route(
            ...
            .route(
                "/api/admin/cases",
                get(cases::get_cases).post(cases::create_case),
            )
            .route(
            ...
            .route(
                "/api/admin/files/{id}/thumbnail",
                get(files::get_file_thumbnail),
            )
            .route("/api/admin/files/{id}", delete(files::delete_file))
            .route(
                "/api/admin/files/user/{user_id}",
                get(files::get_user_files),
            )
            .route(
                "/api/admin/files/{file_id}/associate/{entity_type}/{entity_id}",
                post(files::associate_file).delete(files::disassociate_file),
            )
            .route(
                "/api/admin/files/associated/{entity_type}/{entity_id}",
                get(files::get_associated_files),
            )
            // Network management
            .route("/api/admin/tenant-stats", get(admin::get_all_network_stats))
            .route(
                "/api/admin/tenant-stats/{tenant_id}",
                get(admin::get_network_stats),
            )
            .route(
                "/api/admin/tenant/{tenant_id}/listings",
                get(admin::get_network_listings),
            )
            .route(
                "/api/admin/tenant/{tenant_id}/listings/{listing_id}",
                get(admin::get_listing),
            )
            .route("/api/admin/platform/apps", get(admin::get_platform_apps))
            .route(
                "/api/admin/platform/apps/{instance_id}/domains",
                get(admin::get_app_domains).post(admin::add_app_domain),
            )
            .route(
                "/api/admin/platform/apps/{instance_id}/domains/{domain_name}",
                delete(admin::remove_app_domain),
            )
            // Deployment config mutations (keyed on tenant_id, not instance_id)
            .route(
                "/api/admin/platform/apps/{tenant_id}/account",
                put(admin::link_deployment_account),
            )
            .route(
                "/api/admin/platform/apps/{tenant_id}/purpose",
                put(admin::set_deployment_purpose),
            )
            // Provision a new tenant from scratch (tenant + user + account + app_instances + domain + setup link)
            .route(
                "/api/admin/tenants/provision",
                post(crate::handlers::admin_provision::provision_tenant),
            )
            // Re-provision existing tenant's app instances (calls provision() on all active AtlasApps)
            .route(
                "/api/admin/platform/provision/{tenant_id}",
            ...
            .route(
                "/api/admin/listings",
                get(admin::get_network_listings).post(listings::create_listing),
            )
            .route(
                "/api/admin/listings/{id}",
                get(listings::get_listing_by_id)
                    .put(listings::update_listing)
                    .delete(listings::delete_listing),
            )
            .route(
                "/api/admin/listings/{id}/ab-tests",
                get(crate::handlers::ab_testing::get_listing_ab_tests_admin),
            )
            .route(
                "/api/admin/listings/pending",
                get(admin::list_pending_listings),
            )
            .route(
                "/api/admin/listings/{listing_id}/approve",
                post(admin::approve_listing),
            )
            .route(
                "/api/admin/listings/{listing_id}/reject",
                post(admin::reject_listing),
            )
            // Ad purchase management
            .route(
                "/api/admin/ad-purchases/stats",
                get(admin::get_ad_purchase_stats),
            )
            .route("/api/admin/ad-purchases", get(admin::list_ad_purchases))
            .route(
                "/api/admin/ad-purchases/{purchase_id}",
                get(admin::get_ad_purchase),
            )
            .route(
                "/api/admin/ad-purchases/active",
                get(admin::list_active_ad_purchases),
            )
            .route(
                "/api/admin/ad-purchases/{purchase_id}/cancel",
                post(admin::cancel_ad_purchase),
            )
            // Category management
            .route(
            ...
            .route(
                ...
                get(profiles::get_profile_by_id)
            ...
            .route(
                "/admin/statistics/accounts",
                get(admin::get_account_statistics),
            )
            .route(
                "/admin/statistics/listings",
                get(admin::get_listing_statistics),
            )
            .route(
                "/admin/statistics/ad-purchases",
                get(admin::get_ad_purchase_statistics),
            )
            .route("/admin/reports/activity", get(admin::get_activity_report))
            .route("/admin/reports/revenue", get(admin::get_revenue_report))
            // Analytics
            .route(
                "/api/admin/analytics/business_kpis",
                get(crate::admin::analytics::get_business_kpis),
            )
            .route(
                "/api/admin/analytics/engagement",
                get(crate::admin::analytics::get_engagement),
            )
            .route(
                "/api/admin/analytics/trends",
                get(crate::admin::analytics::get_trends),
            )
            .route(
            ...
            .route(
                ...
                get(crate::admin::billing::list_billing_plans)
                    .post(crate::admin::billing::create_billing_plan),
            )
            .route(
                "/api/admin/billing/plans/{id}",
                put(crate::admin::billing::update_billing_plan)
                    .delete(crate::admin::billing::delete_billing_plan),
            )
            .route(
                "/api/admin/billing/transactions",
                get(crate::admin::billing::list_transactions),
            )
            .route(
                "/api/admin/billing/tenant/{tenant_id}",
                get(crate::admin::billing::get_tenant_ledger),
            )
            .route(
                "/api/admin/billing/tenant/{tenant_id}/subscription/{id}/exemption",
                post(crate::admin::billing::set_subscription_exemption),
            )
            .route(
                "/api/admin/billing/tenant/{tenant_id}/subscription/{id}/suspend",
                post(crate::admin::billing::suspend_subscription),
            )
            .route(
                "/api/admin/billing/tenant/{tenant_id}/subscription/{id}/reactivate",
                post(crate::admin::billing::reactivate_subscription),
            )
            .route(
                "/api/admin/billing/tenant/{tenant_id}/subscription",
                get(crate::admin::billing::get_tenant_subscription),
            )
            // AI Task logs
            .route(
                "/api/admin/ai-tasks/{id}/logs",
                get(crate::admin::ai_tasks::get_task_logs),
            )
            // Developer Console
            .route(
                "/api/admin/developer/tenant/{tenant_id}/api-tokens",
                get(crate::admin::developer_console::list_api_tokens)
                    .post(crate::admin::developer_console::create_api_token),
            )
            .route(
                "/api/admin/developer/tenant/{tenant_id}/api-tokens/{token_id}",
                delete(crate::admin::developer_console::revoke_api_token),
            )
            .route(
                "/api/admin/developer/tenant/{tenant_id}/webhooks",
                get(crate::admin::developer_console::list_webhook_endpoints)
                    .post(crate::admin::developer_console::create_webhook_endpoint),
            )
            .route(
                "/api/admin/developer/tenant/{tenant_id}/webhooks/{endpoint_id}",
                delete(crate::admin::developer_console::delete_webhook_endpoint),
            )
            .route(
                "/api/admin/developer/tenant/{tenant_id}/webhook-deliveries",
                get(crate::admin::developer_console::list_webhook_deliveries),
            )
            ...
            .route(
                "/api/admin/ab-tests/{id}/end",
                axum::routing::post(crate::handlers::ab_testing::end_ab_test),
            )
            ...
            // G-19 Campaigns: list/create campaigns + enrollments + DM tracking
            .merge(crate::admin::campaigns::routes_raw())
            // Integration webhooks (direct mail providers, etc.)
            .merge(crate::admin::integrations::routes_raw())
            ...
            // G-36 — productized growth/incentive program admin API
            .merge(crate::handlers::programs_admin::routes())
    })
}
