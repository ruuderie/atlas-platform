...
async fn provision_and_get_instance_id(
    ...
    assert_eq!(
        resp.status(),
        StatusCode::CREATED,
        "provision must succeed for lifecycle test setup"
    );
...
async fn reset_instance_returns_200_with_active_status() {
    ...
    assert_eq!(
        body["status"], "active",
        "reset response status must be 'active'"
    );
    assert!(
        body.get("instance_id").is_some(),
        "reset response must include instance_id"
    );
    assert!(
        body.get("note").is_some(),
        "reset response must include note field for UI toast"
    );
}
...
async fn delete_instance_row_still_exists_in_db() {
    ...
    assert!(
        row.is_some(),
        "DB row must still exist after soft delete (row is archived, not removed)"
    );
    assert_eq!(
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/api/admin/app-instances/{instance_id}/reprovision-domain"
                ))
                .header("Authorization", format!("Bearer {token}"))
                .header("Host", "localhost")
                .body(Body::empty())
    ...
    let _ = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("Host", "localhost")
                .body(Body::from(
                    json!({ "custom_domain": custom_dom }).to_string(),
                ))
                .unwrap(),
        )
        .await
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/api/admin/app-instances/{fake_id}/reprovision-domain"
                ))
                .header("Authorization", format!("Bearer {token}"))
                .header("Host", "localhost")
                .body(Body::empty())
