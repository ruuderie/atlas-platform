...
use crate::services::{account_service::AccountService, lead_service::LeadService};
...
async fn test_lead_find_duplicate_returns_none_for_unknown_email() {
    ...
    let result =
        LeadService::find_duplicate(&db, tenant.id, Some("nobody@nowhere.invalid"), None, None)
            .await
            .unwrap();
...
async fn test_lead_create_from_import_dedup_pattern() {
    ...
    let existing = LeadService::find_duplicate(&db, tenant.id, Some("import@test.com"), None, None)
        .await
        .unwrap();
    ...
    let found = LeadService::find_duplicate(&db, tenant.id, Some("import@test.com"), None, None)
        .await
        .unwrap();
    assert!(
        found.is_some(),
        "after import, find_duplicate must find the lead"
    );
...
async fn test_lead_disqualify_returns_error_for_converted_lead() {
    ...
    assert!(
        result.is_err(),
        "disqualifying a converted (terminal) lead must fail"
    );
    let err = result.unwrap_err();
    assert!(
        err.contains("terminal"),
        "error must mention terminal state; got: {err}"
    );
}
...
async fn test_lead_disqualify_returns_error_for_foreign_tenant() {
    ...
    assert!(
        result.is_err(),
        "disqualifying another tenant's lead must fail"
    );
}
...
async fn test_account_find_by_domain_finds_exact_match() {
    ...
    let found = AccountService::find_by_domain(&db, tenant.id, &domain)
        .await
        .unwrap();
...
async fn test_account_search_returns_empty_for_no_match() {
    ...
    let results = AccountService::search(&db, tenant.id, "zzz_no_such_account_xyzzy_9999", 10)
        .await
        .unwrap();
