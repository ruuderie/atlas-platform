...
async fn set_pixel(
    ...
    let res = app
        ...
        .oneshot(
            Request::builder()
                .method("PUT")
                .uri(format!("/api/admin/landing-pages/{}/pixels/{}", page_id, pixel_type))
                .header("Host", "localhost")
                .header("Content-Type", "application/json")
                .header("Authorization", format!("Bearer {token}"))
                .body(Body::from(json!({ "enabled": enabled, "snippet": null }).to_string()))
                .unwrap(),
        )
        .await
        .unwrap();
...
async fn test_post_lp_event_rejects_invalid_event_type() {
    ...
    let (status, _) = post_lp_event(&app, json!({
        "app_page_id":  page_id,
        "event_type":   "pageview",        // ← wrong name
        "session_id":   Uuid::new_v4(),
    })).await;

    assert_eq!(status, StatusCode::BAD_REQUEST, "unknown event_type must be rejected");
}
...
async fn test_post_lp_event_accepts_view() {
    ...
    assert_eq!(status, StatusCode::ACCEPTED, "valid view event should be accepted");
...
async fn test_analytics_returns_zeros_for_empty_page() {
    ...
    assert_eq!(body["conv_rate_pct"], 0.0, "0 views → conv rate 0.0, not NaN");
}
...
async fn test_set_pixel_rejects_unknown_provider() {
    ...
    assert_eq!(status, StatusCode::BAD_REQUEST, "unknown pixel type must be rejected");

    let (status, _) = set_pixel(&app, page_id, "GA4", true, &token).await;
    assert_eq!(status, StatusCode::BAD_REQUEST, "wrong case must be rejected");
}
...
async fn test_set_pixel_enables_and_get_pixel_reads_back() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["ga4"]["enabled"], true, "PUT response should reflect new state");

    // GET pixels should now show ga4 enabled
    let (status, body) = get_pixels(&app, page_id, &token).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["ga4"]["enabled"],  true, "ga4 should be persisted as enabled");
    assert_eq!(body["meta"]["enabled"], false, "meta should remain disabled");
}
...
async fn test_set_pixel_preserves_previously_enabled_siblings() {
    ...
    assert_eq!(s, StatusCode::OK);
    assert_eq!(body["ga4"]["enabled"],      true,  "ga4 must survive linkedin toggle");
    assert_eq!(body["linkedin"]["enabled"], true,  "linkedin should now be enabled");
    assert_eq!(body["meta"]["enabled"],     false, "meta should remain untouched");
    ...
    assert_eq!(body["linkedin"]["enabled"], true,  "linkedin must survive ga4 disable");
