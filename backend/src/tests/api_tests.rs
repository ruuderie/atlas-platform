...
use fake::{
    Fake,
    faker::{
        address::en::{CityName, StateAbbr, StreetName, ZipCode},
...
pub async fn setup_test_app() -> (Router, DatabaseConnection) {
    ...
    let opt_docker_compose =
        sea_orm::ConnectOptions::new("postgres://postgres:postgres@localhost:5433/oplydbtest")
            .connect_timeout(std::time::Duration::from_secs(2))
            .to_owned();

    let opt_woodpecker =
        sea_orm::ConnectOptions::new("postgresql://postgres:postgres@database:5432/oplydbtest")
            .connect_timeout(std::time::Duration::from_secs(2))
            .to_owned();
    ...
    let ingress_provisioner =
        Arc::new(crate::services::ingress_provisioner::IngressProvisioner::new());

    let app = api::create_router(db.clone())
        .layer(cors)
        .layer(axum::Extension(webauthn_state))
        .layer(axum::Extension(cors_registry))
        .layer(axum::Extension(ingress_provisioner));
...
async fn test_logout_with_invalid_token() {
    ...
    let response = app
        ...
        .unwrap();
    println!(
        "TEST LOG: from test_logout_with_invalid_token and status: {:?} , body: {:?}",
        response.status(),
        response
    );
...
async fn test_profile_management() {
    ...
    let tenant = test_utils::create_test_tenant(&db).await;
    println!(
        "TEST LOG: from test_profile_management and tenant: {:?}",
        tenant
    );
    println!(
        "TEST LOG: from test_profile_management and tenant: {:?}",
    ...
    println!(
        "TEST LOG: from test_profile_management and token: {:?}",
        token
    );
    ...
    let profile_response = app
        ...
        .unwrap();
    println!(
        "TEST LOG: from test_profile_management and profile_response: {:?}",
        profile_response
    );
    assert_eq!(profile_response.status(), StatusCode::OK);
    //how many profiles are in the response
    let body_bytes = axum::body::to_bytes(profile_response.into_body(), usize::MAX)
        .await
        .unwrap();
    let profiles: Vec<crate::entities::profile::Model> =
        serde_json::from_slice(&body_bytes).unwrap();
    println!(
        "TEST LOG: from test_profile_management and profiles: {:?}",
        profiles.len()
    );
    //print the first profile in the response
    println!(
        "TEST LOG: from test_profile_management and profiles[0]: {:?}",
        profiles[0]
    );
    ...
    println!(
        "TEST LOG: from test_profile_management and profile: {:?}",
        profile
    );
...
async fn test_profile_operations() {
    ...
    let token = login_response["token"].as_str().unwrap();
    println!(
        "TEST LOG: from test_profile_operations and token: {:?}",
        token
    );
    ...
    let profiles: Vec<serde_json::Value> = serde_json::from_slice(
        &axum::body::to_bytes(profiles_response.into_body(), usize::MAX)
            .await
            .unwrap(),
    )
    .unwrap();
    //how many profiles are in the response
    println!(
        "TEST LOG: from test_profile_operations and profiles: {:?}",
        profiles.len()
    );
    //print the first profile in the response
    println!(
        "TEST LOG: from test_profile_operations and profiles[0]: {:?}",
        profiles[0]
    );
...
async fn test_concurrent_logout_requests() {
    ...
    println!(
        "TEST LOG: from test_concurrent_logout_requests and token: {:?}",
        token
    );
    ...
    for response in responses {
        println!(
            "response from test_concurrent_logout_requests: {:?}",
            response
        );
    ...
    assert_eq!(
        success_count + unauthorized_count,
        5,
        "Not all requests returned expected status codes"
    );
}
...
async fn test_listing_crud_operations() {
    ...
    let create_listing_response = app
        .clone()
        .oneshot(
            Request::builder()
                .header("Host", "localhost")
    ...
    let body_bytes = axum::body::to_bytes(create_listing_response.into_body(), usize::MAX)
        .await
        .unwrap();
    let created_listing: crate::entities::listing::Model =
        serde_json::from_slice(&body_bytes).unwrap();

    // Test getting a specific listing
    let get_listing_response = app
        .clone()
        .oneshot(
            Request::builder()
                .header("Host", "localhost")
    ...
    let update_listing_response = app
        .clone()
        .oneshot(
            Request::builder()
                .header("Host", "localhost")
    ...
    let body_bytes = axum::body::to_bytes(update_listing_response.into_body(), usize::MAX)
        .await
        .unwrap();
    let updated_listing: crate::entities::listing::Model =
        serde_json::from_slice(&body_bytes).unwrap();
    assert_eq!(updated_listing.title, updated_title);

    // Test getting all listings for a tenant
    let tenant_listings_response = app
        .clone()
        .oneshot(
            Request::builder()
                .header("Host", "localhost")
    ...
    let body_bytes = axum::body::to_bytes(tenant_listings_response.into_body(), usize::MAX)
        .await
        .unwrap();
    let tenant_listings: Vec<crate::entities::listing::Model> =
        serde_json::from_slice(&body_bytes).unwrap();
    assert!(
        !tenant_listings.is_empty(),
        "No listings returned for tenant"
    );

    // Test deleting the listing
    let delete_listing_response = app
        .clone()
        .oneshot(
            Request::builder()
                .header("Host", "localhost")
    ...
    let get_deleted_listing_response = app
        .clone()
        .oneshot(
            Request::builder()
                .header("Host", "localhost")
...
async fn test_listing_operations() {
    ...
    let mut username = format!("testuser{}", Uuid::new_v4());
    println!(
        "TEST LOG: from test_listing_operations and username: {:?}",
        username
    );
    let (status, registration_body) =
        test_utils::register_test_user(&app, tenant.id, &mut username).await;
    assert_eq!(status, StatusCode::CREATED);
    println!(
        "TEST LOG: from test_listing_operations and registration_body: {:?}",
        registration_body
    );
    ...
    let body_bytes = axum::body::to_bytes(listings_response.into_body(), usize::MAX)
        .await
        .unwrap();
    let listings: Vec<crate::entities::listing::Model> =
        serde_json::from_slice(&body_bytes).unwrap();
...
async fn test_inline_passkey_registration() {
    ...
    assert_eq!(
        webauthn_req.status(),
        StatusCode::OK,
        "Passkey challenge rejected with status {}",
        webauthn_req.status()
    );
}
