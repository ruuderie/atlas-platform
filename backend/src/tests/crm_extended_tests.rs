...
async fn test_crm_leads_lifecycle() {
    ...
    let (status, login_res_a) =
        test_utils::register_test_user(&app, tenant.id, &mut username_a).await;
    ...
    let account_body = axum::body::to_bytes(response.into_body(), usize::MAX)
        .await
        .unwrap();
...
async fn test_crm_cases_and_notes() {
    ...
    let cust_body = axum::body::to_bytes(customer_res.into_body(), usize::MAX)
        .await
        .unwrap();
