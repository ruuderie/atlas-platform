...
pub fn hash_password(password: &str) -> Result<String, bcrypt::BcryptError> {
    ...
    match &result {
        Ok(hash) => {
            tracing::debug!(
                "[{}] Password hashed successfully in {:.2}ms (hash length: {})",
                operation_id,
                duration.as_millis(),
                hash.len()
            );
        ...
        Err(e) => {
            tracing::error!(
                "[{}] Password hashing failed: {:?} (took {:.2}ms)",
                operation_id,
                e,
                duration.as_millis()
            );
...
pub fn verify_password(password: &str, hash: &str) -> Result<bool, bcrypt::BcryptError> {
    let operation_id = Uuid::new_v4();
    tracing::debug!(
        "[{}] Verifying password (password length: {}, hash length: {})",
        operation_id,
        password.len(),
        hash.len()
    );
    ...
    match &result {
        Ok(is_valid) => {
            if *is_valid {
                tracing::info!(
                    "[{}] Password verification successful (took {:.2}ms)",
                    operation_id,
                    duration.as_millis()
                );
            } else {
                tracing::warn!(
                    "[{}] Password verification failed - invalid password (took {:.2}ms)",
                    operation_id,
                    duration.as_millis()
                );
            }
        ...
        Err(e) => {
            tracing::error!(
                "[{}] Password verification error: {:?} (took {:.2}ms)",
                operation_id,
                e,
                duration.as_millis()
            );
...
fn require_jwt_secret() -> Result<String, jsonwebtoken::errors::Error> {
    match env::var("JWT_SECRET") {
        Ok(s) if !s.is_empty() => Ok(s),
        _ if cfg!(test) => Ok("test_secret_for_unit_tests_only".to_string()),
        _ => {
            // Do not panic mid-request — that drops the TCP connection and can
            // leave one-time tokens consumed with no session issued.
            tracing::error!("JWT_SECRET must be set in the environment");
...
pub fn generate_jwt(user: &user::Model) -> Result<String, jsonwebtoken::errors::Error> {
    ...
    tracing::debug!(
        "[{}] JWT claims: sub={}, exp={}, jti={}",
        operation_id,
        claims.sub,
        claims.exp,
        claims.jti.as_ref().unwrap_or(&"none".to_string())
    );
    ...
    match &result {
        Ok(token) => {
            ...
            tracing::info!(
                "[{}] JWT generated successfully for user {} (token: {}, took {:.2}ms)",
                operation_id,
                user.id,
                token_preview,
                duration.as_millis()
        ...
        Err(e) => {
            tracing::error!(
                "[{}] JWT generation failed for user {}: {:?} (took {:.2}ms)",
                operation_id,
                user.id,
                e,
                duration.as_millis()
...
pub fn generate_jwt_admin(user: &user::Model) -> Result<String, jsonwebtoken::errors::Error> {
    ...
    tracing::debug!(
        "[{}] Admin JWT will expire in {} hours",
        operation_id,
        jwt_expiry_hours
    );
    ...
    match &result {
        Ok(token) => {
            ...
            tracing::info!(
                "[{}] Admin JWT generated successfully for user {} (token: {}, took {:.2}ms)",
                operation_id,
                user.id,
                token_preview,
                duration.as_millis()
        ...
        Err(e) => {
            tracing::error!(
                "[{}] Admin JWT generation failed for user {}: {:?} (took {:.2}ms)",
                operation_id,
                user.id,
                e,
                duration.as_millis()
...
pub fn generate_impersonation_jwt(
    ...
    let operation_id = Uuid::new_v4();
    tracing::info!(
        "[{}] Generating IMPERSONATION JWT for user: {} ({}) by Admin: {}",
        operation_id,
        target_user.id,
        target_user.email,
        admin_id
...
pub fn validate_jwt<T: DeserializeOwned>(token: &str) -> Result<T, jsonwebtoken::errors::Error> {
    ...
    match &result {
        Ok(_token_data) => {
            tracing::info!(
                "[{}] JWT validated successfully (type: {}, took {:.2}ms)",
                operation_id,
                std::any::type_name::<T>(),
                duration.as_millis()
            );
        ...
        Err(e) => {
            tracing::warn!(
                "[{}] JWT validation failed: {:?} (took {:.2}ms)",
                operation_id,
                e,
                duration.as_millis()
            );
            ...
            match e.kind() {
                ...
                jsonwebtoken::errors::ErrorKind::InvalidSignature => {
                    tracing::warn!(
                        "[{}] JWT has invalid signature - possible tampering attempt",
                        operation_id
                    );
                }
                _ => {
